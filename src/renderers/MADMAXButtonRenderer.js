...
export class MADMAXButtonRenderer {
  ...
  #letters(ctx,g){
    const size=g.side*.28;
    ctx.fillStyle="#121719";ctx.font=`900 ${size}px "Arial Black", Impact, sans-serif`;
    ctx.textAlign="center";ctx.textBaseline="middle";
    const positions={
      M:[.22,.39],A1:[.5,.22],D:[.78,.39],
      M2:[.22,.68],A2:[.5,.82],X:[.78,.68]
    };
    for(const [label,key] of [["M","M"],["A","A1"],["D","D"],["M","M2"],["A","A2"],["X","X"]]){
      const [u,v]=positions[key];ctx.fillText(label,g.left+g.side*u,g.top+g.side*v);
    }
  }
